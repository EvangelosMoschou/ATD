...
static int design_rrc_taps(double beta, int span, int sps, double* h) {
    if (!h || beta <= 0.0 || beta >= 1.0 || span <= 0 || sps <= 0) return -1;
    ...
    for (int n = 0; n < ntaps; ++n) {
        ...
        const double num = sin(M_PI * t * (1.0 - beta)) + 4.0 * beta * t * cos(M_PI * t * (1.0 + beta));
...
static void conv_real_taps_complex(const Complex* x, size_t nx, const double* h, int nh, Complex* y) {
...
static int export_time_domain_csv(const char* tag, const Complex* before_up, const Complex* after_up, size_t n, size_t max_samples) {
    if (!tag || !before_up || !after_up || n == 0 || max_samples == 0) return -1;
    char name[128];
...
static void fft_radix2(Complex* x, int n, int inverse) {
    int log2n = 0;
    while ((1 << log2n) < n) log2n++;
    for (int i = 0; i < n; i++) {
        int rev = 0;
        for (int j = 0; j < log2n; j++) {
            if (i & (1 << j)) rev |= 1 << (log2n - 1 - j);
        }
        if (i < rev) {
            Complex tmp = x[i];
            x[i] = x[rev];
    ...
    for (int len = 2; len <= n; len <<= 1) {
        ...
        for (int i = 0; i < n; i += len) {
            ...
            for (int j = 0; j < len / 2; j++) {
                ...
                Complex next_w;
                next_w.re = w.re * wlen.re - w.im * wlen.im;
                next_w.im = w.re * wlen.im + w.im * wlen.re;
                w = next_w;
            }
        }
    }
...
static int export_dual_spectrum_csv(
    ...
    double carrier_hz,
    int avg_windows) {
    if (!file_name || !a || !b || fft_len <= 16 || n == 0) return -1;
    const int nfft = fft_len;
    if ((size_t)nfft > n) return -2;

    const size_t total_windows = n / (size_t)nfft;
    int windows_to_use = avg_windows;
    if (windows_to_use < 1) {
        windows_to_use = 1;
    }
    if ((size_t)windows_to_use > total_windows) {
        windows_to_use = (int)total_windows;
    }

    Complex* a_copy = calloc((size_t)nfft, sizeof(Complex));
    Complex* b_copy = calloc((size_t)nfft, sizeof(Complex));
    double* pa_acc = calloc((size_t)nfft, sizeof(double));
    double* pb_acc = calloc((size_t)nfft, sizeof(double));
    if (!a_copy || !b_copy || !pa_acc || !pb_acc) {
        free(a_copy);
        free(b_copy);
        free(pa_acc);
        free(pb_acc);
    ...
    const size_t start_window = (total_windows - (size_t)windows_to_use) / 2u;
    for (int w = 0; w < windows_to_use; ++w) {
        const size_t base = (start_window + (size_t)w) * (size_t)nfft;
        for (int i = 0; i < nfft; ++i) {
            a_copy[i] = a[base + (size_t)i];
            b_copy[i] = b[base + (size_t)i];
        }

        fft_radix2(a_copy, nfft, 0);
        fft_radix2(b_copy, nfft, 0);

        for (int i = 0; i < nfft; ++i) {
            pa_acc[i] += (a_copy[i].re * a_copy[i].re + a_copy[i].im * a_copy[i].im) / (double)nfft;
            pb_acc[i] += (b_copy[i].re * b_copy[i].re + b_copy[i].im * b_copy[i].im) / (double)nfft;
        }
    }
    ...
    for (int k = 0; k < nfft; ++k) {
        ...
        const double pa = pa_acc[ks] / (double)windows_to_use;
        const double pb = pb_acc[ks] / (double)windows_to_use;
...
static int export_rrc_response_csv(const char* tag, const double* taps, int ntaps, int fft_len, double sample_rate_hz, double carrier_hz) {
    if (!tag || !taps || ntaps <= 0 || fft_len <= 16) return -1;
    char name[128];
...
int simulate_modulated_link(const char* tag, const Complex* constellation, int m, int bits_per_symbol, size_t symbols_per_snr, double constellation_capture_db, size_t constellation_points, const LinkOptions* options) {
    if (!options || !tag || !constellation || m <= 1 || bits_per_symbol <= 0 || symbols_per_snr < 10) return -1;
    
    int rc = -1;
    const int sps = options->sps;
    const int filter_span_symbols = options->filter_span_symbols;
    const double rolloff = options->rolloff;
    if (sps <= 1 || filter_span_symbols < 2 || rolloff <= 0.0 || rolloff >= 1.0) return -2;
    if (options->use_rf && (options->sample_rate_hz <= 0.0 || options->carrier_hz <= 0.0)) return -3;
    ...
    double* taps = NULL;
    unsigned* tx_idx = NULL;
    Complex* tx_syms = NULL;
    Complex* up = NULL;
    Complex* tx_wave = NULL;
    Complex* tx_chan = NULL;
    Complex* noisy = NULL;
    Complex* noisy_bb = NULL;
    Complex* rx_wave = NULL;
    FILE* fber = NULL;
    FILE* fconst = NULL;
    ...
    taps = calloc((size_t)ntaps, sizeof(double));
    tx_idx = calloc(symbols_per_snr, sizeof(unsigned));
    tx_syms = calloc(symbols_per_snr, sizeof(Complex));
    up = calloc(up_len, sizeof(Complex));
    tx_wave = calloc(tx_len, sizeof(Complex));
    tx_chan = calloc(tx_len, sizeof(Complex));
    noisy = calloc(tx_len, sizeof(Complex));
    noisy_bb = calloc(tx_len, sizeof(Complex));
    rx_wave = calloc(rx_len, sizeof(Complex));

    if (!taps || !tx_idx || !tx_syms || !up || !tx_wave || !tx_chan || !noisy || !noisy_bb || !rx_wave) {
        rc = -4;
        goto cleanup;
    }
    ...
    fber = fopen(ber_name, "w");
    fconst = fopen(const_name, "w");
    if (!fber || !fconst) {
        rc = -6;
        goto cleanup;
    }
    ...
    for (double ebn0_db = 0.0; ebn0_db <= 20.0 + 1e-12; ebn0_db += 2.0) {
        for (size_t n = 0; n < symbols_per_snr; ++n) {
            tx_idx[n] = prng_uint32() % (uint32_t)m;
            tx_syms[n] = constellation[tx_idx[n]];
        }

        for (size_t i = 0; i < up_len; ++i) { up[i].re = 0.0; up[i].im = 0.0; }
        for (size_t n = 0; n < symbols_per_snr; ++n) up[n * sps] = tx_syms[n];

        conv_real_taps_complex(up, up_len, taps, ntaps, tx_wave);

        for (size_t i = 0; i < tx_len; ++i) tx_chan[i] = tx_wave[i];
        ...
        for (size_t i = 0; i < tx_len; ++i) {
            noisy[i].re = tx_chan[i].re + sigma * prng_gauss();
            noisy[i].im = tx_chan[i].im + sigma * prng_gauss();
        }

        for (size_t i = 0; i < tx_len; ++i) noisy_bb[i] = noisy[i];
        if (options->use_rf) {
        ...
        if (options->export_spectrum && !diagnostics_written && fabs(ebn0_db - options->spectrum_capture_db) < 1e-12) {
            char spec_before[128], spec_after[128];
            snprintf(spec_before, sizeof(spec_before), "spectrum_before_upconversion_%s.csv", tag);
            snprintf(spec_after, sizeof(spec_after), "spectrum_after_upconversion_%s.csv", tag);

            export_time_domain_csv(tag, tx_wave, tx_chan, tx_len, 2048);
            ...
            export_dual_spectrum_csv(spec_before, tx_wave, noisy_bb, tx_len, options->spectrum_fft_len, fs_hz, fc_hz, options->spectrum_avg_windows);
            export_dual_spectrum_csv(spec_after, tx_chan, noisy, tx_len, options->spectrum_fft_len, fs_hz, fc_hz, options->spectrum_avg_windows);
            export_rrc_response_csv(tag, taps, ntaps, options->spectrum_fft_len, fs_hz, fc_hz);
        ...
        for (size_t n = (size_t)discard_syms; n + (size_t)discard_syms < symbols_per_snr; ++n) {
            ...
            unsigned tx_mapped = tx_sym;
            unsigned rx_mapped = rx_sym;
            if (options->use_gray_coding) {
                if (options->ring_bits == 0) {
                    tx_mapped = tx_sym ^ (tx_sym >> 1);
                    rx_mapped = rx_sym ^ (rx_sym >> 1);
                } else {
                    int p = options->ring_bits;
                    int k_bits = bits_per_symbol - p;
                    unsigned mask = (1u << k_bits) - 1;
                    unsigned tx_r = tx_sym >> k_bits;
                    unsigned tx_k = tx_sym & mask;
                    tx_mapped = ((tx_r ^ (tx_r >> 1)) << k_bits) | (tx_k ^ (tx_k >> 1));
                    unsigned rx_r = rx_sym >> k_bits;
                    unsigned rx_k = rx_sym & mask;
                    rx_mapped = ((rx_r ^ (rx_r >> 1)) << k_bits) | (rx_k ^ (rx_k >> 1));
                }
            }

            bit_errors += (unsigned long long)bit_errors_uint(tx_mapped, rx_mapped, bits_per_symbol);
